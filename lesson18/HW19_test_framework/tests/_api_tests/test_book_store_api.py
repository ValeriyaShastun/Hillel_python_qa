import pytest
from lesson18.HW19_test_framework.api_collections.bookstore import BookstoreAPI
from lesson18.HW19_test_framework.api_collections.login import LoginAPI
from http import HTTPStatus
from lesson18.HW19_test_framework.api_collections.logout import LogoutAPI
from lesson18.HW19_test_framework.api_collections.user import UserAPI
import allure



# to launch from IDE and get allure report, add --alluredir=allure_report in additional arguments
# to launch from console:
# pytest --alluredir=lesson18/HW19_test_framework/tests/allure_report lesson18/HW19_test_framework/tests/_api_tests/test_book_store_api.py
# to open allure report run in console:
# allure serve /Users/vshastun/PycharmProjects/HILLEL_project/lesson18/HW19_test_framework/tests/allure_report
@allure.feature("Valeriia Shastun")
...
@allure.feature("Valeriia Shastun")
@pytest.mark.positive
def test_post_login_page(login_api, logout_api):
    """
     ...
     """
    login_page_post_response = login_api["response"]
...
@allure.feature("Valeriia Shastun")
@pytest.mark.positive
def test_get_logout_page(login_api):
    """
...
@allure.feature("Valeriia Shastun")
@pytest.mark.positive
def test_add_book_for_user(login_api, logout_api):
    """
    ...
    bookstore_page_post_response = BookstoreAPI().post_book_for_user(isbn_number_to_add, login_resp["user_id"],
                                                                     login_resp["user_token"])
    ...
    # post-cond
    BookstoreAPI().delete_book_for_user(login_resp["user_id"], login_resp["user_token"], isbn_number_to_add)
    logout_api


@allure.feature("Valeriia Shastun")
@pytest.mark.positive
def test_add_books_for_user(login_api, logout_api):
    """
    ...
    bookstore_page_post_response = BookstoreAPI().post_books_for_user(login_resp["user_id"], login_resp["user_token"],
    ...
    assert bookstore_page_post_response.reason == 'Created'
    # post-cond
    BookstoreAPI().delete_book_for_user(login_resp["user_id"], login_resp["user_token"], isbn_number_to_add_1)
    BookstoreAPI().delete_book_for_user(login_resp["user_id"], login_resp["user_token"], isbn_number_to_add_2)
    logout_api


@allure.feature("Valeriia Shastun")
@pytest.mark.negative
def test_add_same_book_for_user(login_api, logout_api):
    """
    ...
    # add book
    isbn_number_to_add = "9781593275846"
    BookstoreAPI().post_book_for_user(isbn_number_to_add, login_resp["user_id"], login_resp["user_token"],)
    bookstore_page_post_response = BookstoreAPI().post_book_for_user(isbn_number_to_add, login_resp["user_id"],
                                                                     login_resp["user_token"],)
...
@allure.feature("Valeriia Shastun")
@pytest.mark.positive
def test_delete_book_for_user(login_api, logout_api):
    """
     ...
     """
    with allure.step("Add book to user's collection"):
        login_resp = login_api
        isbn_number_to_add = "9781593275846"
        BookstoreAPI().post_book_for_user(isbn_number_to_add, login_resp["user_id"], login_resp["user_token"])
    with allure.step("Delete book from user's collection"):
        bookstore_page_delete_book_response = BookstoreAPI().delete_book_for_user(login_resp["user_id"],
                                                                                  login_resp["user_token"], isbn_number_to_add)
...
@allure.feature("Valeriia Shastun")
@pytest.mark.positive
def test_put_book_for_user(login_api, logout_api):
    """
    ...
    BookstoreAPI().post_book_for_user(isbn_number_to_replace, login_resp["user_id"], login_resp["user_token"])
    # test
    ...
    bookstore_page_put_response = BookstoreAPI().put_book_for_user(isbn_number_to_replace, isbn_number_to_put,
                                                                   login_resp["user_id"], login_resp["user_token"])
    ...
    # post-cond
    BookstoreAPI().delete_book_for_user(login_resp["user_id"], login_resp["user_token"], isbn_number_to_put)
    logout_api


@allure.feature("Valeriia Shastun")
@pytest.mark.negative
def test_delete_user_without_token(login_api, logout_api):
    """
    ...
    user_token = None
    # test
    user_page_delete_response = UserAPI().delete_user(login_resp["user_id"], user_token)
...
@allure.feature("Valeriia Shastun")
@pytest.mark.positive
def test_delete_user(login_api, logout_api):
    """
    ...
    login_resp = login_api
    # test
    user_page_delete_response = UserAPI().delete_user(login_resp["user_id"], login_resp["user_token"])
