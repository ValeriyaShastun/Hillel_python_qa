...
from lesson18.HW19_test_framework.page_objects.home_page.home_page import HomePage
from lesson18.HW19_test_framework.utilities.driver_factory import DriverFactory
...
def failure_tracking_fixture(request, start_driver):
    ...
    if tests_failed_finally - tests_failed_initially == 1:
        if ReadConfig.get_api_or_ui() == "True":
            driver = start_driver
            driver.save_screenshot(f"{ROOT_DIR}/tests/screenshots/{request.node.nodeid}_"
                                         f"{datetime.datetime.now().strftime('%Y-%m-%d - %H:%M')}.jpg")
            allure.attach(driver.get_screenshot_as_png(),
                          name=f"{request.node.nodeid}_{datetime.datetime.now().strftime('%Y-%m-%d - %H:%M')}",
...
def login_api():
    ...
    return {"response":login_page_post_response,"user_id":user_id, "user_token":user_token}


@pytest.fixture()
def logout_api():
    return LogoutAPI().get_logout_page()
